...
const LoginForm = () => {
    const router = useRouter();
    const searchParams = useSearchParams();                          // ← 1. যোগ করো
    const callbackUrl = searchParams.get("callbackUrl") || "/";     // ← 2. callbackUrl নাও
    const [error, setError] = useState("");
    const [loading, setLoading] = useState(false);
    ...
    const handleSubmit = async (e) => {
        ...
        const { error } = await signIn.email({
            email,
            password,
            callbackURL: callbackUrl,                               // ← 3. "/" এর বদলে callbackUrl
        });

        if (error) {
            setError(error.message || "Invalid email or password.");
        } else {
            router.push(callbackUrl);                               // ← 4. callbackUrl এ redirect
        }
